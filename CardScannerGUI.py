...
def process_image(image_path, output_dir, args, unique_id=None):
    try:
        ...
        for contour in contours:
            ...
            prefix = f"{unique_id}_" if unique_id is not None else ""
            output_filename = f"{prefix}{base_name}_card_{cards_found}_{card_type}{ext}"
...
class ProcessingWorker(QObject):
    ...
    def run(self):
        # Limit the number of workers to the number of CPUs or a safe default
        max_workers = os.cpu_count() or 4
        # Don't spin up more workers than files
        max_workers = min(max_workers, len(self.file_paths))

        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks with a unique index to prevent filename collisions
            futures = [
                executor.submit(process_image, fp, self.output_dir, self.args, i)
                for i, fp in enumerate(self.file_paths)
            ]

            # Emit progress as each task completes
